...
\textbf{Grupo} nr. & 04
\\\hline
A72053 & Ângela Andrade Carvalho Fernandes
\\
A65432 & João Manuel Martins Cerqueira
\\
A71506 & Sónia Catarina Guerra Costa
\end{tabular}
\end{center}
...
\begin{code}
inv x = p2 . (cataNat (split (either(const(1-x)) ((uncurry(*)).(split(const (1-x)) p1))) (either (fromIntegral.one) (uncurry(+))) ))


---- Testes com o QuickCheck

prop_inv1 :: Float -> Integer -> Property 
prop_inv1 x n = (x >= 1) ==> (inv x 0 == 1) 

prop_inv2 :: Float -> Integer -> Property
prop_inv2 x n = (x >= 1) ==> (inv 1 10 == inv 1 20) 
\end{code}
\subsubsection*{Cálculos Auxiliares} % (fold)

\begin{Verbatim}[fontsize=\small]
aux_inv :: Float -> Integer -> Float
aux_inv x 0 = (1-x)
aux_inv x (n+1) = (1-x)^(n+1+1) 

inv_p1 :: Float -> Integer -> Float
inv_p1 x 0 = 1
inv_p1 x (n+1) = aux_inv x n + inv_p x n
\end{Verbatim}
\textbf{Aplicando as Leis}
\begin{eqnarray*}
\start
  |lcbr (inv_p1 x . in = [h1,h2] . F <inv_p1 x, aux_inv x> )(aux_inv x . in = [k1,k2] . F <inv_p1 x, aux_inv x>)| 
%
\just={ definição de F }
%
  |lcbr (inv_p1 x . in = [h1,h2] . (id + inv_p1 x))(aux_inv x . in = [k1,k2] . (id + aux_inv x))|
%
\just={ Absorção-+, def in}
%
  |lcbr (lcbr (inv_p1 x . const 0 = h1 )(inv_p1 x.succ=h2 .(inv_p1 x)))(lcbr (aux_inv x . const 0 = k1 )(aux_inv x . succ = k2 .(aux_inv x)))|
      .
%
\just={ Igualdade Extensional}
%
  |lcbr (lcbr (inv_p1 x (const 0.x) = h1 x)(inv_p1 x (n+1) = h2 (inv_p1 x) (n+1)))(lcbr (aux_inv x (const 0.x) = k1 x)(aux_inv x (n+1) = k2 (aux x) (n+1)))|
      .
%
\just={ Natural-cons}
%
  |lcbr (lcbr (inv_p1 x (const 0) = const 1 )(inv_p1 x (n+1) = h2 (inv_p1 x) (n+1)))(lcbr (aux_inv x (const 0) = const (1-x))(aux_inv x (n+1) = k2 (aux_inv x) (n+1)))|
        .
%
\just={ Verifica-se que }
%
  <inv\_p1 x, aux\_inv x> = (| <[h1,h2],[k1, k2]> |) 
  .
%  
\just={------------------}
% 
   Onde: |lcbr (lcbr (h1 = const(1))(h2 = uncurry(*).<const(1-x),p1>))(lcbr (k1 = const(1-x))(k2 = uncurry(+)))|
        .
%
\end{eqnarray*}
...
-- gene é a condicional de McCarthy
gene2 = cond (uncurry(&&).(split(not.sep.p1) (p2.p2))) ((1+).(p1.p2)) (p1.p2) 


wrapper = p1 
worker = (cataList (either (split zero true) (split gene2 (sep.p1)))) 

---- Testes com o QuickCheck

prop_wc1 :: [Char] -> Property 
prop_wc1 l = (l/=[]) ==> ((wc_w_final "as ") == 1) 

prop_wc2 :: [Char] -> Property 
prop_wc2 l = (l/=[]) ==> ((wc_w_final []) == 0) 

prop_wc3 :: [Char] -> Property 
prop_wc3 l = (l/=[]) ==> ((wc_w_final " as  b   ") == 2) 
\end{code}
...
\begin{eqnarray*}
\start
  |lcbr (f = wc)(g = lookahead)| 
%
\just={ Lei fokkinga }
%
  |lcbr (wc . in = [h1,h2] . F <f,g>)(lookahead . in = [k1,k2] . F <f,g>)|
%
\just={ definição de F }
%
  |lcbr (wc . in = [h1,h2] . (id + id x <wc, lookahead>))(lookahead . in  = [k1,k2] . (id + id x <wc, look>) )|
%
\just={ def in}
%
  |lcbr (lcbr (wc . nil = h1)(wc . cons = h2 (id x <wc, lookahead>)))(lcbr (lookahead . nil = k1)(lookahead . cons = k2 (id x <wc, lookahead>)))|
      .
%
\just={ def nil e cons}
%
  |lcbr (lcbr (wc [] = const 0)(wc (c:l) = h2 (c,(wc t, lookahead t))))(lcbr (lookahead [] = True)(lookahead (c:t) = k2 (c,(wc t, lookahead t))))|
      .
%
\just={ Lei fokkinga }
%
  <wc, lookahead> = (|<[const 0, gene2], [true, sep.p1]>|)
%
\just={ Lei da Troca}
%
  <wc, lookahead> = (|[<const 0,true>,<gene2,sep.p1>]|) onde : 
%  
  |lcbr (lcbr (h1 = const 0)(h2 = uncurry(&&).<not.sep.p1, p2.p2>))(lcbr (k1 = True)(k2 = sep.p1))|
        .
%
\end{eqnarray*}
...
outB_tree Nil               = Left ()
outB_tree (Block a ((b,c):t)) = Right(a, ((b,c):t))
...
inordB_tree = cataB_tree (either nil ((uncurry (++). (id >< concat.(map(
  cons))))))

cataBTree g = cataB_tree g
\end{code}
...
--largestBlock = 
--    let b = (uncurry max ) . (split (maximum.(map p2)) (length))
--    in cataBTree (either (const 0) (uncurry max.(id >< b)) )
...
lsplitB_tree :: [Int] -> Either () ([Int], [(Int, [Int])])
lsplitB_tree [] = Left ()
lsplitB_tree x = Right (separaM x)

separaM :: Ord t => [t] -> ([t], [(t, [t])])
separaM[x] = ([], [(x,[])]) 

separaM(x:y:t) = let
    maior = max x y
    menor = min x y
    lista_menores = filter(\n-> n < menor) t
    e = filter(\n-> n > menor && n < maior) t
    lista_maiores = filter(\n-> n > maior) t
    in (lista_menores, [(menor,e),(maior, lista_maiores)])

gene = (either nil ((uncurry (++). (id >< concat.(map(cons))))))

qSortB_tree = (hyloB_tree gene lsplitB_tree)
...
\begin{code}
permuta [] = return []
permuta lista = do { (h,t) <- getR lista; 
                    resultado <- permuta t; 
                    return (h:resultado) }
...
\just={ Universal-cata }
%
  permuta . in = in . (id+id><f)
%
\just={ def in}
%
  permuta . [nil, cons] = [nil, cons] (id+id><f)
%
\just={ def Fusão-+}
%
  [permuta . nil, permuta . cons] = [nil.id , cons.(id+id><f)]
      .
%
\just={ Universal-+ }
%
  |lcbr (permuta . nil = ([nil, cons] . (id + id >< f)) . i1)(permuta .cons = ([nil, cons] . (id + id >< f)) . i2
)|
%
\just={ Absorção-+}
%
...
\just={ igualdade extensional }
%
  |lcbr (permuta.nil () = ([nil.id,cons.(id><f)].i1)()) (permuta.cons (h,t) = ([nil.id,cons.(id><f)].i2) (h,t) )| 
%
\just={ Cancelamento-+}
%
  |lcbr (permuta (nil ()) =  (nil.id) ())(permuta (cons (h,t)) = (cons.(id >< f)) (h,t))| 
% 
\just={ Def comp}
%
  |lcbr (permuta (nil ()) = nil(id())) ())(permuta (cons (h,t)) = (cons.(id >< f)) (h,t))| 
...
%
  |lcbr (permuta (nil()    ) = nil(id()) ) (permuta (cons(h,t)) = cons ((id><f)) (h,t) )| 
%
\just={ Definição ao ponto}
% 
  |lcbr (permuta (nil()) = nil ())(permuta (cons (h,t)) = cons (id h,(f t)))| 
%  
\just={ Definição id}
%
  |lcbr (permuta (nil()) = nil(id()) )(permuta(cons(h,t)) =(cons.(id><f)) (h,t) )| 
...
\just={ Definição ao ponto}
%
  |lcbr (permuta (nil()) = nil ())(permuta (cons (h,t)) = cons (h,(f t)))| 
%
\just={ Def-cons, Def-nil}
%
  |lcbr (permuta [] = [])(permuta (h:t) = (h:(f t)))| 
%  
\end{eqnarray*}
\subsubsection*{Código}
\begin{code}
